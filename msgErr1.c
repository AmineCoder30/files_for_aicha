...
char *func_envErr(char **amgt)
{
	...
	amgt--;
	len = string_lenght(name) + string_lenght(hist_str) + string_lenght(amgt[0]) + 45;
...
char *errNum_1(char **amgt)
{
	...
	len = string_lenght(name) + string_lenght(amgt[0]) + 13;
	error = malloc(sizeof(char) * (len + 1));
	if (!error)
		return (NULL);
...
char *exitErr_num2(char **amgt)
{
	...
	len = string_lenght(name) + string_lenght(hist_str) + string_lenght(amgt[0]) + 27;
	error = malloc(sizeof(char) * (len + 1));
	if (!error)
	{
		free(hist_str);
...
char *syntxErr_num2(char **amgt)
{
	...
	len = string_lenght(name) + string_lenght(hist_str) + string_lenght(amgt[0]) + 33;
	error = malloc(sizeof(char) * (len + 1));
	if (!error)
	{
		free(hist_str);
