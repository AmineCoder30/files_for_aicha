#include "shell.h"
int (*get_builtin(char *command))(char **args, char **front);
int exit_shell(char **args, char **front);
int change_fileDir(char **args, char __attribute__((__unused__)) **front);
int helper_shell(char **args, char __attribute__((__unused__)) **front);

/**
 * get_builtin - Matches a command with a corresponding
 *               shellby builtin function.
 * @command: The command to match.
 *
 * Return: A function pointer to the corresponding builtin.
 */
int (*get_builtin(char *command))(char **args, char **front)
{
	bltnType funcs[] = {
		{ "exit", exit_shell },
		{ "env", envir_shell },
		{ "setenv", set_envir_shell },
		{ "unsetenv", unset_envir_shell },
		{ "cd", change_fileDir },
		{ "alias", set_shellVar },
		{ "help", helper_shell },
		{ NULL, NULL }
	};
	int i;

	for (i = 0; funcs[i].name; i++)
	{
		if (compare_string(funcs[i].name, command) == 0)
			break;
	}
	return (funcs[i].f);
}

/**
 * exit_shell - Causes normal process termination
 *                for the shellby shell.
 * @args: An array of arguments containing the exit value.
 * @front: A double pointer to the beginning of args.
 *
 * Return: If there are no arguments - -3.
 *         If the given exit value is invalid - 2.
 *         O/w - exits with the given status value.
 *
 * Description: Upon returning -3, the program exits back in the main function.
 */
int exit_shell(char **args, char **front)
{
	int i, len_of_int = 10;
	unsigned int num = 0, max = 1 << (sizeof(int) * 8 - 1);

	if (args[0])
	{
		if (args[0][0] == '+')
		{
			i = 1;
			len_of_int++;
		}
		for (; args[0][i]; i++)
		{
			if (i <= len_of_int && args[0][i] >= '0' && args[0][i] <= '9')
				num = (num * 10) + (args[0][i] - '0');
			else
				return (func_createErr(--args, 2));
		}
	}
	else
	{
		return (-3);
	}
	if (num > max - 1)
		return (func_createErr(--args, 2));
	args -= 1;
	fn_to_free_arg(args, front);
	fn_to_free_envir();
	func_free_alsList(aliases);
	exit(num);
}

/**
 * change_fileDir - Changes the current directory of the shellby process.
 * @args: An array of arguments.
 * @front: A double pointer to the beginning of args.
 *
 * Return: If the given string is not a directory - 2.
 *         If an error occurs - -1.
 *         Otherwise - 0.
 */
int change_fileDir(char **args, char __attribute__((__unused__)) **front)
{
	char **dir_info, *new_line = "\n";
	char *oldpwd = NULL, *pwd = NULL;
	struct stat dir;

	oldpwd = getcwd(oldpwd, 0);
	if (!oldpwd)
		return (-1);

	if (args[0])
	{
		if (*(args[0]) == '-' || compare_string(args[0], "--") == 0)
		{
			if ((args[0][1] == '-' && args[0][2] == '\0') ||
					args[0][1] == '\0')
			{
				if (fn_to_get_envir("OLDPWD") != NULL)
					(chdir(*fn_to_get_envir("OLDPWD") + 7));
			}
			else
			{
				free(oldpwd);
				return (func_createErr(args, 2));
			}
		}
		else
		{
			if (stat(args[0], &dir) == 0 && S_ISDIR(dir.st_mode)
					&& ((dir.st_mode & S_IXUSR) != 0))
				chdir(args[0]);
			else
			{
				free(oldpwd);
				return (func_createErr(args, 2));
			}
		}
	}
	else
	{
		if (fn_to_get_envir("HOME") != NULL)
			chdir(*(fn_to_get_envir("HOME")) + 5);
	}

	pwd = getcwd(pwd, 0);
	if (!pwd)
		return (-1);

	dir_info = malloc(sizeof(char *) * 2);
	if (!dir_info)
		return (-1);

	dir_info[0] = "OLDPWD";
	dir_info[1] = oldpwd;
	if (set_envir_shell(dir_info, dir_info) == -1)
		return (-1);

	dir_info[0] = "PWD";
	dir_info[1] = pwd;
	if (set_envir_shell(dir_info, dir_info) == -1)
		return (-1);
	if (args[0] && args[0][0] == '-' && args[0][1] != '-')
	{
		write(STDOUT_FILENO, pwd, string_lenght(pwd));
		write(STDOUT_FILENO, new_line, 1);
	}
	free(oldpwd);
	free(pwd);
	free(dir_info);
	return (0);
}

/**
 * helper_shell - Displays information about shellby builtin commands.
 * @args: An array of arguments.
 * @front: A pointer to the beginning of args.
 *
 * Return: If an error occurs - -1.
 *         Otherwise - 0.
 */
int helper_shell(char **args, char __attribute__((__unused__)) **front)
{
	if (!args[0])
		help_all();
	else if (compare_string(args[0], "alias") == 0)
		alsHelper();
	else if (compare_string(args[0], "cd") == 0)
		chdirHelper();
	else if (compare_string(args[0], "exit") == 0)
		extHelper();
	else if (compare_string(args[0], "env") == 0)
		envirHelper();
	else if (compare_string(args[0], "setenv") == 0)
		setEnvirHelper();
	else if (compare_string(args[0], "unsetenv") == 0)
		unSetEnvirHelper();
	else if (compare_string(args[0], "help") == 0)
		hpHelper();
	else
		write(STDERR_FILENO, name, string_lenght(name));

	return (0);
}
