...
int unset_envir_shell(char **amgt, char __attribute__((__unused__)) **ourfront);
...
int set_envir_shell(char **amgt, char __attribute__((__unused__)) **ourfront)
{
	...
	new_value = malloc(string_lenght(amgt[0]) + 1 + string_lenght(amgt[1]) + 1);
	if (!new_value)
		return (func_createErr(amgt, -1));
...
/**
 * unset_envir_shell - this function delete envir.
* @amgt: array of amgt.
 * @ourfront: dounle ptr.
 * Return: return -1 or 0.
 */
