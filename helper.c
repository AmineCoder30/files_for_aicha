...
/**
 * fn_to_free_arg - Frees up memory taken by args.
...
char *get_env_value(char *beginning, int len)
{
	...
	if (var_addr)
	{
		...
		while (*temp != '=')
			temp++;
		temp++;
		replacement = malloc(string_lenght(temp) + 1);
		if (replacement)
...
void replace_var(char **line, int *exe_ret)
{
	...
	for (j = 0; old_line[j]; j++)
	{
				...
				old_line[j + 1] != ' ')
		{
			...
			new_line = malloc(j + string_lenght(replacement)
					  + string_lenght(&old_line[k]) + 1);
			if (!line)
				return;
			new_line[0] = '\0';
